...
def write_sitemaps():
    ...
    for loc, last_mod in sitemap_urls():
        ...
        if url_count % max_urls == 0:
            ...
            sitemap_index.write('<sitemap><loc>https://chroniclingamerica.loc.gov/%s</loc></sitemap>\n' % sitemap_file)

        # add a url to the sitemap
        sitemap.write("<url><loc>https://chroniclingamerica.loc.gov%s</loc><lastmod>%s</lastmod></url>\n" % (loc, rfc3339(last_mod)))
        url_count += 1
